export const dashboardTileStyles = {
  ...
  tileTitle: 'text-body font-medium leading-tight text-foreground',
  tileInfoButton:
    'inline-flex h-6 w-6 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted/60',
  tileMetric: 'mt-3 text-display font-bold leading-none tracking-tight text-foreground',
  tileSecondary: 'mt-2 min-h-5 text-label font-normal leading-5 text-muted-foreground',
  tileVisualWrap: 'mt-4 rounded-md border border-border/70 bg-muted/20 p-2',
  tileFooter: 'mt-3 flex min-h-5 items-center justify-between gap-2',
  tileFooterText: 'text-xs font-normal text-muted-foreground',
  tileMicroLabel: 'text-micro font-normal text-muted-foreground',
  tagPill: 'inline-flex h-6 items-center rounded-full border px-2.5 text-micro font-semibold',
  ...
  barShape:
    'w-full rounded-md transition-transform duration-150 origin-bottom group-hover/tile:scale-[1.03]',
  valuePill:
