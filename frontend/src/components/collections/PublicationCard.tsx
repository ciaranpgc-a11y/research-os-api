...
export function PublicationCard(props: PublicationCardProps) {
            ...
            {collectionMemberships.length > 0 && (
              <span className="flex items-center gap-1 flex-wrap min-w-0">
                {collectionMemberships.map((c) => (
                  <span
                    key={`${c.id}:${c.subcollection_id ?? 'root'}`}
                    ...
                    title={c.subcollection_name ? `${c.name} / ${c.subcollection_name}` : c.name}
                  >
                    {c.subcollection_name ? `${c.name} / ${c.subcollection_name}` : c.name}
                  </span>
                ))}
              </span>
            )}
  ...
  const currentCollection = collections.find((collection) => collection.id === currentCollectionId) ?? null
  const currentSubcollection = props.subcollectionId
    ? (subcollectionsMap.get(currentCollectionId) ?? []).find((sub) => sub.id === props.subcollectionId) ?? null
    : null
  return (
    <div
      draggable
    ...
    >
      ...
      >
        ...
        <div className="mt-0.5 flex items-center gap-2 min-w-0">
          <div className="text-xs text-muted-foreground truncate">{metaLine}</div>
          {currentCollection && currentSubcollection ? (
            <span
              className="inline-flex max-w-[160px] items-center gap-1 rounded px-1.5 py-0.5 text-[0.75rem] leading-[1.1rem] font-medium truncate"
              style={{
                backgroundColor: `${COLLECTION_COLOUR_HEX[currentCollection.colour]}16`,
                color: COLLECTION_COLOUR_HEX[currentCollection.colour],
              }}
              title={currentSubcollection.name}
            >
              <span
                className="h-1.5 w-1.5 shrink-0 rounded-full"
                style={{ backgroundColor: COLLECTION_COLOUR_HEX[currentCollection.colour] }}
                aria-hidden="true"
              />
              <span className="truncate">{currentSubcollection.name}</span>
            </span>
          ) : null}
        </div>
      </div>
