...
def _build_publication_paper_payload(
    *,
    ...
    payload = {
        ...
        "references": references,
        "reference_id_map": (
            {k: v for k, v in parsed_payload.get("reference_id_map", {}).items()}
        ...
        "provenance": {
            ...
            or None,
            "pmcid": str(parsed_payload.get("pmcid") or "").strip() or None,
        },
    }
...
def _tei_visible_numeric_bibr_label(node: ET.Element | None) -> str | None:
...
def _tei_node_text_with_citations(node: ET.Element | None) -> str:
    ...
    def _walk(el: ET.Element) -> None:
        ...
        if tag == "ref":
            ...
            if ref_type == "bibr":
                visible_label = _tei_visible_numeric_bibr_label(el)
                if visible_label:
...
def _tei_section_blocks(node: ET.Element | None) -> list[str]:
    ...
    for child in list(node):
        ...
        if tag == "listbibl":
            ...
            continue
        plain_text = _tei_node_text(child)
        if _is_figure_legend_paragraph(plain_text or ""):
            continue
        raw_text = _tei_node_text_with_citations(child)
...
def _extract_biblstruct_fields(node: ET.Element) -> dict[str, Any]:
    """Extract structured fields from a GROBID ``<biblStruct>`` element."""
    fields: dict[str, Any] = {}
    xml_id = str(node.attrib.get("{http://www.w3.org/XML/1998/namespace}id") or node.attrib.get("xml:id") or "").strip()
    if xml_id:
        fields["xml_id"] = xml_id
    label_el = _tei_first_direct_child(node, "label")
    if label_el is not None:
        label_text = _tei_node_text(label_el).strip()
        if label_text:
            fields["original_label"] = label_text
    analytic = _tei_first_direct_child(node, "analytic")
    monogr = _tei_first_direct_child(node, "monogr")
    title_source = analytic if analytic is not None else monogr
    if title_source is not None:
        for child in list(title_source):
            if _xml_local_name(getattr(child, "tag", "")) == "title":
                t = _tei_node_text(child).strip()
                if t:
                    fields["title"] = t
                    break
    authors: list[str] = []
    author_sources = [analytic, monogr]
    ...
    if monogr is not None:
        for child in list(monogr):
            if _xml_local_name(getattr(child, "tag", "")) == "title":
                jt = _tei_node_text(child).strip()
                if jt and jt != fields.get("title"):
                    fields["journal"] = jt
                    break
        imprint = _tei_first_direct_child(monogr, "imprint")
        if imprint is not None:
            for child in list(imprint):
                tag = _xml_local_name(getattr(child, "tag", ""))
                if tag == "date":
                    when = str(child.attrib.get("when") or "").strip()
                    if when:
                        fields["year"] = when[:4]
                elif tag == "biblscope":
                    ...
                    if unit == "volume" and val:
                        ...
                        if pg_from and pg_to:
                            fields["pages"] = f"{pg_from}-{pg_to}"
                        elif val:
                            fields["pages"] = val
    for idno_node in node.iter():
        if _xml_local_name(getattr(idno_node, "tag", "")) != "idno":
            continue
    ...
    for container in candidate_iterables:
        for node in container.iter():
            ...
            if marker in seen or _is_publication_paper_boilerplate_block(raw_text):
                continue
            seen.add(marker)
            ref_index = len(references) + 1
            entry: dict[str, Any] = {
                "id": f"paper-reference-{ref_index}",
                "label": f"Reference {ref_index}",
                "raw_text": raw_text,
            }
            if tag_lower == "biblstruct":
                ...
                if original_label:
                    ...
                    num_match = re.search(r"\d+", xml_id)
                    if num_match:
                        entry["label"] = num_match.group(0)
...
def _grobid_table_node_to_html(figure_node: ET.Element) -> str | None:
    """Convert GROBID ``<table><row><cell>`` markup inside a ``<figure type="table">`` to standard HTML."""
    table_el = _tei_first_direct_child(figure_node, "table")
    if table_el is None:
        return None
    rows: list[str] = []
    for child in list(table_el):
        if _xml_local_name(getattr(child, "tag", "")) != "row":
            continue
        role = str(child.attrib.get("role") or "").strip().lower()
        cell_tag = "th" if role == "header" else "td"
        cells: list[str] = []
        for cell_node in list(child):
            if _xml_local_name(getattr(cell_node, "tag", "")) != "cell":
                continue
            colspan = str(cell_node.attrib.get("cols") or "").strip()
            attrs = f' colspan="{int(colspan)}"' if colspan.isdigit() and int(colspan) > 1 else ""
            text = _tei_node_text(cell_node)
            escaped = (
                text.replace("&", "&amp;")
                .replace("<", "&lt;")
                .replace(">", "&gt;")
            )
            cells.append(f"<{cell_tag}{attrs}>{escaped}</{cell_tag}>")
        if cells:
...
def _extract_publication_paper_assets_from_tei(
    ...
    for node in root.iter():
        ...
        if is_table:
            ...
            if grobid_html:
                parsed_asset["structured_html"] = grobid_html
...
_PVALUE_SIGNIFICANT_PATTERN = re.compile(
    r"^[<≤]?\s*0?\.(\d+)$|^[<≤]\s*0?\.?0+1$",
)
...
def _publication_table_is_pvalue_significant(text: str) -> bool:
    ...
    if not clean:
        return False
    # Handle prefix like "p=" or "P ="
    clean = re.sub(r"^[pP]\s*[=:]\s*", "", clean)
    # "<0.001", "<.01", "≤0.05" etc — always significant
    if clean.startswith(("<", "≤")):
        remainder = clean.lstrip("<≤").strip()
        try:
            return float(remainder) <= 0.05
        except (ValueError, OverflowError):
            return False
    # Plain numeric: "0.001", "0.05", ".03"
    try:
    ...
    *,
    ...
    for block in blocks:
        ...
        for row in list(block.get("rows") or []):
            ...
            parts.append(
                f"<tr>{_publication_table_render_cells(row_cells, pvalue_column=pvalue_column)}</tr>"
            )
        parts.append("</tbody>")
    return "".join(parts)
...
def _canonicalize_docling_table_html(
    ...
    *,
    ...
    total_columns = max(
        (_publication_table_row_cell_total(row["cells"]) for row in rows),
        default=0,
    )
    header_rows: list[list[dict[str, Any]]] = []
    body_rows: list[dict[str, Any]] = []
...
def _pmc_archive_primary_xml_member(
    ...
    def _sort_key(member: tarfile.TarInfo) -> tuple[int, int, str]:
        ...
        return (0 if lower_name.endswith(".nxml") else 1, len(name), lower_name)
...
def _extract_structured_publication_assets_from_pmc_archive_content(
            ...
            for node in root.iter():
                ...
                if local_name == "fig":
                    ...
                    for candidate in node.iter():
                        if (
                            _publication_table_xml_local_name(getattr(candidate, "tag", ""))
                            == "table"
                        ):
                            table_element = candidate
                            break
    ...
    return (
        _merge_publication_paper_asset_collections(parsed_assets=figures, file_assets=[]),
        _merge_publication_paper_asset_collections(parsed_assets=tables, file_assets=[]),
    )
...
def _publication_paper_figure_image_metrics(
    ...
    if not isinstance(asset, dict):
        return None
    raw = str(asset.get("image_data") or "").strip()
    if not raw.startswith("data:") or ";base64," not in raw:
        return None
    prefix, encoded = raw.split(";base64,", 1)
    mime_type = prefix.removeprefix("data:").strip().lower()
    try:
...
def _publication_paper_payload_needs_asset_enrichment(
    ...
    *,
    ...
    has_low_quality_figures = _publication_paper_assets_include_low_quality_figures(figures)
    has_low_quality_tables = _publication_paper_assets_include_low_quality_tables(tables)
    enrichment_status = str(provenance.get("asset_enrichment_status") or "").strip().upper() or None
...
def _overlay_pmc_archive_content_onto_structured_paper(
    *,
    parsed_payload: dict[str, Any],
    pmcid: str,
) -> dict[str, Any]:
    ...
    if not archive_content:
        enriched_payload = dict(parsed_payload)
        enriched_payload["pmcid"] = pmcid
        return enriched_payload

    enriched_payload = dict(parsed_payload)
    pmc_references = _extract_publication_paper_references_from_pmc_archive_content(
        archive_content
    )
    if pmc_references:
        enriched_payload["references"] = pmc_references

    pmc_figures, pmc_tables = _extract_structured_publication_assets_from_pmc_archive_content(
        archive_content
    )
    if pmc_figures:
        enriched_payload["figures"] = _merge_publication_paper_asset_collections(
    ...
    if pmc_tables:
        enriched_payload["tables"] = _merge_publication_paper_asset_collections(
    ...
    enriched_payload["pmcid"] = pmcid
    if pmc_references or pmc_figures or pmc_tables:
        base_generation_method = str(parsed_payload.get("generation_method") or "").strip()
        enriched_payload["generation_method"] = (
            f"{base_generation_method}+pmc_native_assets_v1"
            if base_generation_method
            else "pmc_native_assets_v1"
        )
    return enriched_payload
...
def _extract_structured_publication_paper_with_best_available_parser(
    *,
    ...
    try:
        ...
        if pmcid:
            try:
                return _overlay_pmc_archive_content_onto_structured_paper(
                    parsed_payload=grobid_payload,
                    pmcid=pmcid,
                )
            except Exception as exc:
                logger.warning("PMC archive overlay skipped for %s: %s", pmcid, exc)
                grobid_payload["pmcid"] = pmcid
    ...
    if grobid_error is not None:
        raise grobid_error
    raise PublicationConsoleValidationError(
        "Structured paper parsing was unavailable for this publication."
    )
...
def get_publication_paper_model(*, user_id: str, publication_id: str, force_reparse: bool = False) -> dict[str, Any]:
    create_all_tables()
    source_state = _load_publication_paper_source_state(
        user_id=user_id, publication_id=publication_id
    )
    ...
    with session_scope() as session:
        ...
        if row is None:
            ...
            needs_reseed = (
                (force_reparse and current_status != RUNNING_STATUS)
                or current_hash != source_signature
            ...
            if needs_reseed:
                ...
                has_viewable_pdf
                and current_status != RUNNING_STATUS
