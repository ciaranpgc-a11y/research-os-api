...
def _is_stale(*, computed_at: datetime | None, ttl_seconds: int, now: datetime | None = None) -> bool:
...
def _resolve_work_or_raise(session, *, user_id: str, publication_id: str, for_update: bool = False) -> Work:
    ...
    if work is None:
        raise PublicationConsoleNotFoundError(f"Publication '{publication_id}' was not found.")
    return work
...
def _latest_metric_for_work(session, *, work_id: str) -> MetricsSnapshot | None:
    rows = session.scalars(select(MetricsSnapshot).where(MetricsSnapshot.work_id == work_id)).all()
    if not rows:
        return None
...
def _latest_metric_for_work_at_or_before(session, *, work_id: str, cutoff: datetime) -> MetricsSnapshot | None:
...
def _extract_counts_by_year(snapshot: MetricsSnapshot | None, *, now_year: int) -> dict[int, int]:
    if snapshot is None:
        return {}
...
def _estimate_window_citations(yearly_counts: dict[int, int], *, start: datetime, end: datetime, now: datetime) -> int:
...
def _build_publication_summary(work: Work, *, citations_total: int) -> dict[str, Any]:
    ...
    publication_type = str(work.publication_type or "").strip() or str(work.work_type or "").strip()
    return {
        "id": str(work.id),
        ...
        "keywords_json": [str(item).strip() for item in (work.keywords or []) if str(item).strip()],
...
def _extract_openalex_work_record(*, work: Work, mailto: str | None) -> tuple[dict[str, Any] | None, str | None]:
...
def _extract_authors_from_pubmed(pmid: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for author_node in root.findall(".//AuthorList/Author"):
        ...
        if collective:
            ...
            display_name = " ".join([part for part in [fore_name, last_name] if part]).strip() or last_name or fore_name or initials
        ...
        authors.append({"name": display_name, "orcid_id": None, "affiliations": author_affiliations})

    return authors, [affiliations_index[name] for name in affiliations_order]

def _extract_authors_from_crossref(doi: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    payload = _request_json_with_retry(
    ...
    for item in authors_raw:
        ...
        display_name = str(item.get("name") or "").strip() or " ".join([part for part in [given, family] if part]).strip()
        display_name = re.sub(r"\s+", " ", display_name).strip()
        if not display_name:
            continue
...
def _extract_authors_from_openalex(work_record: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        if isinstance(institutions, list):
            for institution in institutions:
                if not isinstance(institution, dict):
                    continue
                inst_name = re.sub(r"\s+", " ", str(institution.get("display_name") or "").strip())
                country_code = str(institution.get("country_code") or "").strip().upper() or None
                if not inst_name:
                    continue
...
def _get_executor() -> ThreadPoolExecutor:
    ...
    with _executor_lock:
        if _executor is None:
            _executor = ThreadPoolExecutor(max_workers=_max_workers(), thread_name_prefix="pub-console")
        return _executor
    ...
    def _run() -> None:
        try:
            ...
            logger.exception(
                ...
                extra={"kind": kind, "user_id": user_id, "publication_id": publication_id},
            )
        finally:
            with _inflight_lock:
...
def _portfolio_context(
    ...
    *,
    ...
    for work in works:
        ...
        at_12 = _latest_metric_for_work_at_or_before(session, work_id=str(work.id), cutoff=cutoff_12)
        total = int(latest.citations_count or 0) if latest is not None else int(work.citations_total or 0)
        total = max(0, total)

        if latest is not None and at_12 is not None:
            last12 = max(0, int(latest.citations_count or 0) - int(at_12.citations_count or 0))
        elif latest is not None:
            yearly = _extract_counts_by_year(latest, now_year=now.year)
            last12 = _estimate_window_citations(yearly, start=cutoff_12, end=now, now=now)
        else:
            last12 = 0
    ...
    rank_total = next((index for index, item in enumerate(total_by_work, start=1) if item[0] == publication_id), None)
    rank_12m = next((index for index, item in enumerate(last12_by_work, start=1) if item[0] == publication_id), None)

    share_total = round((paper_total_citations / portfolio_total) * 100.0, 2) if portfolio_total > 0 else 0.0
    share_12m = round((paper_last_12m / portfolio_last12) * 100.0, 2) if portfolio_last12 > 0 else 0.0
...
def _fetch_openalex_citing_summaries(*, openalex_work_id: str | None, mailto: str | None) -> dict[str, Any]:
    ...
    for page in range(1, _openalex_citing_pages() + 1):
        ...
        for item in results:
            ...
            journal = re.sub(r"\s+", " ", str(host.get("display_name") or "").strip()) if isinstance(host, dict) else ""
            if journal:
            ...
            if isinstance(authorships, list):
                for authorship in authorships:
                    ...
                    for institution in institutions:
                        if not isinstance(institution, dict):
                            continue
                        code = str(institution.get("country_code") or "").strip().upper()
                        if code:
            ...
            if isinstance(ids, dict):
                pmid = _normalize_pmid(_extract_pmid_from_text(str(ids.get("pmid") or "")) or str(ids.get("pmid") or ""))
            ...
            citing_papers.append(
                {
                    ...
                    "pmid": pmid,
                    "citations_total": max(0, int(_safe_int(item.get("cited_by_count")) or 0)),
                }
            )
    ...
    top_journals = [{"name": name, "count": count} for name, count in journal_counter.most_common(5)]
    top_countries = [{"name": name, "count": count} for name, count in country_counter.most_common(5)]
    citing_papers.sort(key=lambda item: int(item.get("citations_total") or 0), reverse=True)
...
def _build_impact_payload(session, *, user_id: str, publication_id: str) -> dict[str, Any]:
    ...
    at_12 = _latest_metric_for_work_at_or_before(session, work_id=publication_id, cutoff=cutoff_12)
    at_24 = _latest_metric_for_work_at_or_before(session, work_id=publication_id, cutoff=cutoff_24)

    citations_total = int(latest.citations_count or 0) if latest is not None else int(work.citations_total or 0)
    ...
    if not yearly_counts and citations_total > 0:
        bucket = int(work.year) if isinstance(work.year, int) and 1900 <= int(work.year) <= now.year else now.year
        yearly_counts[bucket] = citations_total
    if yearly_counts and citations_total > sum(yearly_counts.values()):
        yearly_counts[now.year] = yearly_counts.get(now.year, 0) + (citations_total - sum(yearly_counts.values()))

    if latest is not None and at_12 is not None:
        citations_last_12 = max(0, int(latest.citations_count or 0) - int(at_12.citations_count or 0))
    else:
        citations_last_12 = _estimate_window_citations(yearly_counts, start=cutoff_12, end=now, now=now)

    if at_12 is not None and at_24 is not None:
        citations_prev_12 = max(0, int(at_12.citations_count or 0) - int(at_24.citations_count or 0))
    else:
        citations_prev_12 = _estimate_window_citations(yearly_counts, start=cutoff_24, end=cutoff_12, now=now)

    yoy_pct = None
    if citations_prev_12 > 0:
        yoy_pct = round(((citations_last_12 - citations_prev_12) / citations_prev_12) * 100.0, 1)
    ...
    compact_openalex_id = resolved_id or _extract_openalex_work_id(work.openalex_work_id) or _extract_openalex_work_id(str((record or {}).get("id") or ""))
    citing = _fetch_openalex_citing_summaries(openalex_work_id=compact_openalex_id, mailto=mailto)
...
def _extract_key_points_from_abstract(abstract: str | None) -> dict[str, str]:
    ...
    heading_pattern = re.compile(r"(?i)\b(background|objective|objectives|aim|aims|purpose|methods?|results?|conclusions?)\s*[:.-]")
    ...
    if matches:
        ...
        for index, match in enumerate(matches):
            ...
            if body:
                sections[label] = body
        return {
            "objective": sections.get("objective") or sections.get("objectives") or sections.get("aim") or sections.get("aims") or sections.get("purpose") or sections.get("background") or default["objective"],
            "methods": sections.get("method") or sections.get("methods") or default["methods"],
            "main_findings": sections.get("result") or sections.get("results") or default["main_findings"],
            "conclusion": sections.get("conclusion") or sections.get("conclusions") or default["conclusion"],
        }

    sentences = _split_sentences(compact)
    objective = _find_sentence(sentences, [r"\b(aim|objective|purpose)\b", r"\bwe (investigated|evaluated|assessed|examined)\b"])
    methods = _find_sentence(sentences, [r"\b(method|methods)\b", r"\b(randomized|retrospective|prospective|cohort|meta-analysis|systematic review)\b", r"\bwe (conducted|performed|analysed|analyzed)\b"])
    findings = _find_sentence(sentences, [r"\b(result|results)\b", r"\bwe found\b", r"\bshowed\b", r"\bdemonstrated\b"])
    conclusion = _find_sentence(sentences, [r"\bconclusion\b", r"\bwe conclude\b", r"\bthis suggests\b", r"\bthese findings indicate\b"])
...
def _build_reuse_suggestions(*, title: str, journal: str, trajectory: str, key_points: dict[str, str]) -> list[str]:
    suggestions: list[str] = []
    if key_points.get("objective") and key_points["objective"] != "Not stated in abstract.":
        suggestions.append("Use for introduction framing and problem statement context.")
    ...
    if key_points.get("main_findings") and key_points["main_findings"] != "Not stated in abstract.":
        suggestions.append("Use in discussion to benchmark outcomes against prior evidence.")
    if trajectory in {"ACCELERATING", "CONSISTENT"}:
        suggestions.append("Prioritise as a core citation in high-salience manuscript sections.")
    if trajectory == "DECLINING":
        suggestions.append("Use selectively and pair with newer corroborating citations.")
    if journal and journal != "Not available":
        suggestions.append(f"Track follow-up papers in {journal} for updated evidence.")
    if not suggestions:
        suggestions.append(f"Use '{title}' as supplementary background until richer metadata is available.")
...
def _build_caution_flags(*, abstract: str | None, key_points: dict[str, str]) -> list[str]:
    flags: list[str] = []
    text = re.sub(r"\s+", " ", str(abstract or "").strip()).lower()
    if not text:
        return ["No abstract available for extractive analysis."]

    if not re.search(r"\b(n\s*=\s*\d+|\d+\s+(patients|participants|subjects|cases))\b", text):
    ...
    if not re.search(r"\b(outcome|endpoint|mortality|effect|improv|benefit|risk)\b", text):
...
def _build_ai_payload(*, publication: dict[str, Any], impact_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    acceleration = _safe_float(impact_payload.get("acceleration_citations_per_month")) or 0.0

    per_year = impact_payload.get("per_year") if isinstance(impact_payload.get("per_year"), list) else []
    trajectory = _normalize_trajectory(_classify_trajectory(per_year))

    summary_parts = [f"{title} has {total} total citations, with {last12} citations in the last 12 months."]
    if prev12 > 0 and yoy is not None:
        summary_parts.append(f"This is {yoy}% versus the previous 12-month window ({prev12} citations).")
    elif prev12 == 0 and last12 > 0:
        summary_parts.append("No prior 12-month baseline is available for direct YoY comparison.")
    else:
        summary_parts.append("Recent citation momentum is currently limited.")
    summary_parts.append(f"Estimated acceleration is {round(acceleration, 2)} citations per month.")
    summary_parts.append(f"Trajectory classification: {trajectory.replace('_', ' ').title()}.")

    key_points = _extract_key_points_from_abstract(publication.get("abstract"))
    reuse = _build_reuse_suggestions(title=title, journal=journal, trajectory=trajectory, key_points=key_points)
    caution = _build_caution_flags(abstract=publication.get("abstract"), key_points=key_points)
...
def _load_impact_cache(session, *, user_id: str, publication_id: str, for_update: bool = False) -> PublicationImpactCache | None:
...
def _load_ai_cache(session, *, user_id: str, publication_id: str, for_update: bool = False) -> PublicationAiCache | None:
...
def _enqueue_authors_if_needed(*, user_id: str, publication_id: str) -> bool:
    with session_scope() as session:
        work = _resolve_work_or_raise(session, user_id=user_id, publication_id=publication_id, for_update=True)
        status = _normalize_status(work.authors_status)
        now = _utcnow()
        has_cached = isinstance(work.authors_json, list) and len(work.authors_json) > 0
        stale = _is_stale(computed_at=_coerce_utc_or_none(work.authors_computed_at), ttl_seconds=_authors_ttl_seconds(), now=now)
    ...
    return _submit_background_job(kind="authors", user_id=user_id, publication_id=publication_id, fn=_run_authors_hydration_job)
...
def _enqueue_impact_if_needed(*, user_id: str, publication_id: str) -> bool:
    with session_scope() as session:
        ...
        row = _load_impact_cache(session, user_id=user_id, publication_id=publication_id, for_update=True)
        now = _utcnow()
        if row is None:
            ...
            stale = _is_stale(computed_at=_coerce_utc_or_none(row.computed_at), ttl_seconds=_impact_ttl_seconds(), now=now)
    ...
    return _submit_background_job(kind="impact", user_id=user_id, publication_id=publication_id, fn=_run_impact_compute_job)


def _enqueue_ai_if_needed(*, user_id: str, publication_id: str) -> bool:
    with session_scope() as session:
        _resolve_work_or_raise(session, user_id=user_id, publication_id=publication_id)
        row = _load_ai_cache(session, user_id=user_id, publication_id=publication_id, for_update=True)
        now = _utcnow()
        if row is None:
            ...
            payload = row.payload_json if isinstance(row.payload_json, dict) else {}
            stale = _is_stale(computed_at=_coerce_utc_or_none(row.computed_at), ttl_seconds=_ai_ttl_seconds(), now=now)
            status = _normalize_status(row.status)
            should_enqueue = (not payload or stale) and status != RUNNING_STATUS
            if should_enqueue:
                row.status = RUNNING_STATUS
    ...
    return _submit_background_job(kind="ai", user_id=user_id, publication_id=publication_id, fn=_run_ai_compute_job)
...
def _run_authors_hydration_job(*, user_id: str, publication_id: str) -> None:
    with session_scope() as session:
        ...
        if result["status"] == READY_STATUS:
            ...
            if openalex_work_id and not _extract_openalex_work_id(work.openalex_work_id):
                work.openalex_work_id = openalex_work_id
            session.flush()
            return

        work.authors_status = FAILED_STATUS
        work.authors_last_error = "Could not resolve authors from PubMed, Crossref, or OpenAlex."
        session.flush()
...
def _run_impact_compute_job(*, user_id: str, publication_id: str) -> None:
    ...
    try:
        with session_scope() as session:
            payload = _build_impact_payload(session, user_id=user_id, publication_id=publication_id)
            work = _resolve_work_or_raise(session, user_id=user_id, publication_id=publication_id, for_update=True)
            row = _load_impact_cache(session, user_id=user_id, publication_id=publication_id, for_update=True)
            if row is None:
                row = PublicationImpactCache(owner_user_id=user_id, publication_id=publication_id)
                session.add(row)
                session.flush()
            ...
            row.computed_at = now
            row.updated_at = now
            work.citations_total = max(0, int(_safe_int(payload.get("citations_total")) or 0))
            openalex_work_id = _extract_openalex_work_id(str(((payload.get("metadata") or {}).get("openalex_work_id")) or ""))
            if openalex_work_id and not _extract_openalex_work_id(work.openalex_work_id):
                work.openalex_work_id = openalex_work_id
            session.flush()
        ...
        with session_scope() as session:
            ...
            row = _load_impact_cache(session, user_id=user_id, publication_id=publication_id, for_update=True)
            if row is None:
                row = PublicationImpactCache(owner_user_id=user_id, publication_id=publication_id, payload_json={}, computed_at=None)
                session.add(row)
                session.flush()
...
def _run_ai_compute_job(*, user_id: str, publication_id: str) -> None:
    ...
    try:
        with session_scope() as session:
            ...
            latest = _latest_metric_for_work(session, work_id=publication_id)
            citations_total = int(latest.citations_count or 0) if latest is not None else int(work.citations_total or 0)
            publication = _build_publication_summary(work, citations_total=max(0, int(citations_total)))

            impact_row = _load_impact_cache(session, user_id=user_id, publication_id=publication_id)
            impact_payload = impact_row.payload_json if impact_row is not None and isinstance(impact_row.payload_json, dict) else {}
            if not impact_payload:
                impact_payload = _build_impact_payload(session, user_id=user_id, publication_id=publication_id)
                if impact_row is None:
                    impact_row = PublicationImpactCache(owner_user_id=user_id, publication_id=publication_id)
            ...
            ai_payload = _build_ai_payload(publication=publication, impact_payload=impact_payload)
            row = _load_ai_cache(session, user_id=user_id, publication_id=publication_id, for_update=True)
            if row is None:
                row = PublicationAiCache(owner_user_id=user_id, publication_id=publication_id)
                session.add(row)
                session.flush()
        ...
        with session_scope() as session:
            ...
            row = _load_ai_cache(session, user_id=user_id, publication_id=publication_id, for_update=True)
            if row is None:
                row = PublicationAiCache(owner_user_id=user_id, publication_id=publication_id, payload_json={}, computed_at=None)
                session.add(row)
                session.flush()
            row.status = FAILED_STATUS
            row.last_error = str(exc)[:2000]
...
def get_publication_authors(*, user_id: str, publication_id: str) -> dict[str, Any]:
    ...
    with session_scope() as session:
        ...
        stale = _is_stale(computed_at=computed_at, ttl_seconds=_authors_ttl_seconds(), now=now)
...
def get_publication_impact(*, user_id: str, publication_id: str) -> dict[str, Any]:
    ...
    with session_scope() as session:
        ...
        latest = _latest_metric_for_work(session, work_id=publication_id)
        citations_total = int(latest.citations_count or 0) if latest is not None else int(work.citations_total or 0)
        row = _load_impact_cache(session, user_id=user_id, publication_id=publication_id)
        now = _utcnow()

        if row is None:
            payload = _empty_impact_payload(work=work, citations_total=max(0, citations_total))
            status = RUNNING_STATUS
            computed_at = None
            stale = True
            last_error = None
            ...
            stale = _is_stale(computed_at=computed_at, ttl_seconds=_impact_ttl_seconds(), now=now)
...
def get_publication_ai_insights(*, user_id: str, publication_id: str) -> dict[str, Any]:
    ...
    with session_scope() as session:
        ...
        if row is None:
            ...
            status = _normalize_status(row.status)
            computed_at = _coerce_utc_or_none(row.computed_at)
            stale = _is_stale(computed_at=computed_at, ttl_seconds=_ai_ttl_seconds(), now=now)
            last_error = str(row.last_error or "").strip() or None
            if (stale or not computed_at) and status != RUNNING_STATUS:
                enqueue = True
                status = RUNNING_STATUS
...
def upload_publication_file(*, user_id: str, publication_id: str, filename: str, content_type: str | None, content: bytes) -> dict[str, Any]:
    create_all_tables()
    if not content:
...
def link_publication_open_access_pdf(*, user_id: str, publication_id: str) -> dict[str, Any]:
    create_all_tables()
    with session_scope() as session:
        user = _resolve_user_or_raise(session, user_id)
        work = _resolve_work_or_raise(session, user_id=user_id, publication_id=publication_id)
        doi = _normalize_doi(work.doi)
        if not doi:
            raise PublicationConsoleValidationError("DOI is required to search for open-access PDF links.")

        email = _unpaywall_email(user_email=user.email)
        if not email:
            raise PublicationConsoleValidationError("UNPAYWALL_EMAIL or account email is required for Unpaywall requests.")
...
def delete_publication_file(*, user_id: str, publication_id: str, file_id: str) -> dict[str, Any]:
    create_all_tables()
    with session_scope() as session:
        _resolve_work_or_raise(session, user_id=user_id, publication_id=publication_id)
        row = session.scalars(
        ...
        if row is None:
            raise PublicationConsoleNotFoundError(f"Publication file '{file_id}' was not found.")
    ...
    if source == FILE_SOURCE_USER_UPLOAD and storage_key:
        try:
            ...
            logger.warning("publication_file_delete_disk_warning", extra={"file_id": file_id, "storage_key": storage_key})

    return {"deleted": True}


def get_publication_file_download(*, user_id: str, publication_id: str, file_id: str) -> dict[str, Any]:
    create_all_tables()
    with session_scope() as session:
        _resolve_work_or_raise(session, user_id=user_id, publication_id=publication_id)
        row = session.scalars(
        ...
        if not path.exists() or not path.is_file():
            raise PublicationConsoleNotFoundError("Uploaded file bytes were not found on disk.")
...
def trigger_publication_authors_hydration(*, user_id: str, publication_id: str) -> dict[str, Any]:
    create_all_tables()
    return {"enqueued": bool(_enqueue_authors_if_needed(user_id=user_id, publication_id=publication_id))}


def trigger_publication_impact_recompute(*, user_id: str, publication_id: str) -> dict[str, Any]:
    create_all_tables()
    return {"enqueued": bool(_enqueue_impact_if_needed(user_id=user_id, publication_id=publication_id))}


def trigger_publication_ai_recompute(*, user_id: str, publication_id: str) -> dict[str, Any]:
    create_all_tables()
    return {"enqueued": bool(_enqueue_ai_if_needed(user_id=user_id, publication_id=publication_id))}
