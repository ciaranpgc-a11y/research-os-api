"""FastAPI router for Extract access control — mounted under /v1/extract/."""

from fastapi import APIRouter, Request
from fastapi.responses import JSONResponse, Response
from pydantic import BaseModel

from research_os.extract_auth import service
...
def extract_user_login(body: UserLoginRequest):
...
def extract_auth_session(request: Request):
    token = _extract_token(request)
    if not token:
        return JSONResponse({"detail": "Authentication required"}, status_code=401)
    user = service.get_session_user(token)
...
def extract_auth_logout(request: Request):
    token = _extract_token(request)
    if token:
...
def extract_admin_login(body: AdminLoginRequest):
...
def extract_admin_list_codes(request: Request):
...
@router.post("/auth/codes", status_code=201)
def extract_admin_create_code(body: CreateCodeRequest, request: Request):
    guard = _require_admin(request)
    if isinstance(guard, JSONResponse):
        return guard
...
@router.delete("/auth/codes/{code_id}", status_code=204)
def extract_admin_revoke_code(code_id: str, request: Request):
    guard = _require_admin(request)
    if isinstance(guard, JSONResponse):
        return guard
